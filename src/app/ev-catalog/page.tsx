...
const EVCatalogPage = () => {
  ...
  useEffect(() => {
    const fetchCatalogs = async () => {
      try {
        const response = await fetch('http://localhost:8000/api/ev-catalog/');
          ...
          <div className="space-y-16">
            ...
            <div>
              <div className="bg-gray-50 rounded-[4rem] p-8 md:p-20 border border-gray-100 shadow-inner overflow-hidden">
                <Flipbook pdfUrl={activeCatalog.url.startsWith('http') ? activeCatalog.url : `http://localhost:8000${activeCatalog.url}`} />
              </div>
            </div>

            <div className="flex justify-center">
              <a
                href={activeCatalog.url.startsWith('http') ? activeCatalog.url : `http://localhost:8000${activeCatalog.url}`}
                download
